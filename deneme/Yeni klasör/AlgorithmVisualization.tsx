...
const AlgorithmVisualization: React.FC<ArrayVisualizationProps> = ({
  ...
  const startVisualization = () => {
    if (sorting) return; // Zaten çalışıyorsa çık
    setSorting(true);
    setCurrentStep(0);
    
    // Algoritma tipine göre uygun görselleştirmeyi başlat
    try {
      // Arama algoritmaları
      if (algorithmType === 'linear_search') {
        visualizeLinearSearch();
      } else if (algorithmType === 'binary_search') {
        visualizeBinarySearch();
      }
      // Sıralama algoritmaları
      else if (algorithmType === 'bubble_sort') {
        visualizeBubbleSort();
      } else if (algorithmType === 'selection_sort') {
        visualizeSelectionSort();
      } else if (algorithmType === 'insertion_sort') {
        visualizeInsertionSort();
      } else if (algorithmType === 'merge_sort') {
        visualizeMergeSort();
      } else if (algorithmType === 'quick_sort') {
        visualizeQuickSort();
      } else if (algorithmType === 'heap_sort') {
        visualizeHeapSort();
      } else if (algorithmType === 'counting_sort') {
        visualizeCountingSort();
      } else if (algorithmType === 'radix_sort') {
        visualizeRadixSort();
      } else if (algorithmType === 'shell_sort') {
        visualizeShellSort();
      }
      // Veri yapıları
      else if (algorithmType === 'singly_linked_list') {
        // Tek yönlü bağlı liste için
        visualizeSinglyLinkedList();
      } else if (algorithmType === 'doubly_linked_list') {
        // Çift yönlü bağlı liste için
        visualizeDoublyLinkedList();
      } else {
        setExplanationText('Bu algoritma tipi henüz desteklenmiyor.');
        setSorting(false);
      }
    } catch (error) {
      console.error('Görselleştirme başlatma hatası:', error);
  ...
  // Dairesel Bağlı Liste Görselleştirmesi - Bu işlevsellik kaldırıldı
  // Artık CircularLinkedListVisualization.tsx dosyasında bulunmaktadır
  ...
  const performCircularLinkedListDemo = async () => {
    ...
    // Dairesel bağlı liste artık ayrı bir bileşene taşındı
    const circularOperations = [
      { name: 'Yeni Bileşen Bilgisi', action: async () => {
        setExplanationText('Dairesel bağlı liste görselleştirmesi artık CircularLinkedListVisualization.tsx dosyasında yer alıyor.');
        addLogMessage('Dairesel bağlı liste için ayrı bileşen kullanın.');
        await wait(speed);
      }}
    ];
